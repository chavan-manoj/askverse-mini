...
def main():
    ...
    while True:
        system = input("\nChoose the system (wiki|tavily|arxiv|docs|ensemble|apis|verse) or (quit|q) to exit: ").strip().lower()
        
        if system in ("q", "quit"):
            print("Thank you for using AskVerse Mini!")
            break
        elif system not in ("wiki", "tavily", "arxiv", "docs", "ensemble", "apis", "verse"):
